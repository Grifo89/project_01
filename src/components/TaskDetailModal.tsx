...
import { db, Task, ProjectMember, Priority } from '../services/db';
import { Icon } from './Icon';
import {
  X, Calendar, AlertCircle, CheckCircle,
...
export const TaskDetailModal = ({ task, onClose, onUpdate }: TaskDetailModalProps) => {
  ...
  const fetchData = async () => {
    try {
      const [subtasksData, commentsData, teamData] = await Promise.all([
        ...
        db.getProjectMembers(task.projectId),   // ← was db.getProjectTeam()
      ]);
  ...
  const handleToggleAssignee = async (authUid: string) => {
    const isAssigned = assigneeIds.includes(authUid);
    const newIds = isAssigned
      ? assigneeIds.filter(id => id !== authUid)
      : [...assigneeIds, authUid];
  ...
  const handleAddComment = async (e: Event) => {
    ...
    // Phase 1 will supply the real auth UID here; for now use a placeholder
    const actorId = 'anonymous';
    await db.addComment(task.id, actorId, newComment);
    setNewComment('');
  ...
  // Find a member by authUid for comment display
  const getMember = (uid: string) => team.find(m => m.authUid === uid);
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {comments.map(c => {
                    ...
                    const displayName = member?.displayName || c.userId || 'Unknown';
                    const photoUrl = member?.photoUrl ?? undefined;
                    return (
                      <div key={c.id} className="flex gap-4">
                        <Avatar displayName={displayName} photoUrl={photoUrl} size="sm" />
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-bold text-app-text-primary">{displayName}</span>
                            <span className="text-[10px] text-app-text-secondary font-medium">{new Date(c.createdAt).toLocaleDateString()}</span>
                          </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {team.filter(m => assigneeIds.includes(m.authUid)).map(m => (
                        <div
                          key={m.authUid}
                          className="flex items-center gap-2 p-1 pr-3 rounded-full border bg-primary/10 border-primary text-primary shadow-sm"
                        >
                          <Avatar displayName={m.displayName} photoUrl={m.photoUrl ?? undefined} size="xs" />
                          <span className="text-[10px] font-bold whitespace-nowrap">{m.displayName}</span>
                          <button
                            onClick={(e) => { e.preventDefault(); handleToggleAssignee(m.authUid); }}
                      ...
                      {assigneeIds.length === 0 && (
                        <p className="text-[10px] text-app-text-secondary italic">
                          {team.length === 0 ? 'Sign in to assign members' : 'No one assigned'}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <Modal isOpen={isAddAssigneeModalOpen} onClose={() => setIsAddAssigneeModalOpen(false)} title="Assign Team Members">
        <div className="space-y-4" onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {team.map(m => {
              const isAssigned = assigneeIds.includes(m.authUid);
              return (
                <button
                  key={m.authUid}
                  onClick={() => handleToggleAssignee(m.authUid)}
                ...
                >
                  <div className="flex items-center gap-3">
                    <Avatar displayName={m.displayName} photoUrl={m.photoUrl ?? undefined} size="sm" />
                    <div className="text-left">
                      <p className="text-sm font-bold text-app-text-primary">{m.displayName}</p>
                      <p className="text-[10px] text-app-text-secondary">{m.role}</p>
                    </div>
                  </div>
                  {isAssigned ? (
            ...
            {team.length === 0 && (
              <p className="text-center text-app-text-secondary py-4 text-sm">Sign in to manage assignees.</p>
            )}
          </div>
          <button
