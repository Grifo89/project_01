...
import { User as UserIcon, Calendar, Tag, AlertCircle, ChevronDown } from 'lucide-preact';
...
export const TaskForm = ({ users = [], onSubmit, onCancel, initialData }: TaskFormProps) => {
  ...
  const handleSubmit = (e: Event) => {
    ...
    onSubmit({ title, description, tag, tagVariant, dueDate, priority, assigneeId: assigneeId || undefined });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        ...
        <input
          ...
          className="w-full px-5 py-4 bg-app-background border border-app-border rounded-2xl text-app-text-primary text-lg font-semibold outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-app-text-secondary/40"
          required
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full px-5 py-4 bg-app-background border border-app-border rounded-2xl text-app-text-primary outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all min-h-[100px] resize-none placeholder:text-app-text-secondary/40"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full px-5 py-3.5 bg-app-background border border-app-border rounded-2xl text-app-text-primary font-medium outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="UI, BUG..."
              className="w-full px-4 py-3.5 bg-app-background border border-app-border rounded-2xl text-app-text-primary font-medium outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all placeholder:text-app-text-secondary/40"
            />
          </div>
          <div className="space-y-2">
            ...
            <div className="relative">
              <select
                value={tagVariant}
                onChange={(e) => setTagVariant((e.target as HTMLSelectElement).value)}
      ...
      <div className="flex gap-4 pt-4">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 px-6 py-4 bg-app-surface border border-app-border rounded-2xl text-sm font-bold text-app-text-secondary hover:bg-app-background transition-all"
        >
          Discard
        </button>
        <button
          type="submit"
          className="flex-1 px-6 py-4 bg-primary text-white rounded-2xl text-sm font-bold shadow-xl shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 transition-all"
        >
          Create Task
        </button>
      </div>
