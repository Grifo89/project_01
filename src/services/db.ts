...
class DatabaseService {
  ...
  private async persist() {
    if (!this.db) return;
    try {
      const tables = ['projects', 'columns', 'project_members', 'sprints', 'tasks', 'subtasks', 'comments', 'activities'];
  ...
  async init() {
    ...
    if (projectCount === 0) {
      ...
      if (Object.keys(tablesData).length > 0) {
        // Restore from existing IndexedDB snapshot (returning user)
        this.db.exec('PRAGMA foreign_keys = OFF;');
        for (const [tableName, rows] of Object.entries(tablesData)) {
          if (tableName === 'users' || tableName === 'project_users') continue;
        ...
        // Fresh install — try /seed.json first (generated by `npm run seed`),
        // then fall back to the inline performSeed() if the file is absent.
        const seededFromJson = await this.loadSeedJson();
        if (!seededFromJson) {
    ...
    // Primary persistence trigger: visibilitychange fires reliably before tab close
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden' && this.isDirty) this.persist();
    });
    // Secondary fire-and-hope fallback
  ...
  /**
   * Fetches /seed.json (generated by `npm run seed`) and inserts all rows
   * into the SQLite database. Returns true if the file was found and loaded,
   * false if the file is absent (404) or malformed.
   *
   * Rows are inserted with INSERT OR IGNORE so this is safe to call on a
   * partially-populated database without creating duplicates.
   */
  private async loadSeedJson(): Promise<boolean> {
    try {
      const res = await fetch('/seed.json');
      if (!res.ok) return false;
      const payload = await res.json();
      if (!payload?.tables) return false;

      console.log(`🌱 Loading seed data from /seed.json (generated ${payload.generated_at})`);
      this.db.exec('PRAGMA foreign_keys = OFF;');

      const knownTables = ['projects', 'columns', 'project_members', 'sprints', 'tasks', 'subtasks', 'comments', 'activities'];
      for (const tableName of knownTables) {
        const rows: any[] = payload.tables[tableName] ?? [];
        for (const row of rows) {
          const keys = Object.keys(row);
          ...
          try {
            this.db.exec({ sql: `INSERT OR IGNORE INTO ${tableName} (${keys.join(', ')}) VALUES (${placeholders})`, bind: values });
          } catch (e) {
            console.warn(`seed.json: failed to insert row into ${tableName}:`, e);
          }
        }
      }
      ...
      // Set browser flags so the app boots directly into the logged-in state
      localStorage.setItem('isLoggedIn', 'true');
      localStorage.setItem('hasSeenLanding', 'true');

      console.log('✅ seed.json loaded successfully');
      return true;
    } catch (e) {
      console.warn('seed.json not found or invalid — falling back to inline seed:', e);
      return false;
    }
  }

  /**
   * Clears all tables, reloads /seed.json (or falls back to performSeed),
   * then re-persists to IndexedDB.
   *
   * Call from the browser console:  db.resetAndReseed()
   * Useful during development to get a clean slate with fresh seed data.
   */
  async resetAndReseed(): Promise<void> {
    await this.init();
    this.db.exec('PRAGMA foreign_keys = OFF;');
    this.db.exec('DELETE FROM activities; DELETE FROM comments; DELETE FROM subtasks; DELETE FROM tasks; DELETE FROM sprints; DELETE FROM project_members; DELETE FROM columns; DELETE FROM projects;');
    this.db.exec('PRAGMA foreign_keys = ON;');
    ...
    await this.persist();
    console.log('✅ Database reset and reseeded. Reload the page to see changes.');
  }

  async manualSeed() {
    await this.init();
    this.db.exec('DELETE FROM project_members; DELETE FROM sprints; DELETE FROM comments; DELETE FROM subtasks; DELETE FROM activities; DELETE FROM tasks; DELETE FROM columns; DELETE FROM projects;');
    await performSeed(this.db);
    await this.persist();
    return true;
  }
  ...
  async getProjects(): Promise<Project[]> {
    await this.init();
    const res: Project[] = [];
    this.db.exec({ sql: 'SELECT * FROM projects ORDER BY created_at DESC', callback: (r: any[]) => res.push({ id: r[0], name: r[1], description: r[2], color: r[3], startDate: r[4], endDate: r[5], sprintStartDay: r[6], sprintDurationWeeks: r[7], createdAt: r[8] }) });
    return res;
  }
  ...
  async addProject(p: any) {
    await this.init();
    const id = generateId();
    this.db.exec({ sql: 'INSERT INTO projects (id, name, description, color, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?)', bind: [id, p.name, p.description || '', p.color || '#3b82f6', p.startDate || new Date().toISOString().split('T')[0], p.endDate || null] });
    ...
    // Note: project membership is established in Phase 1 via addProjectMember() after auth is wired
    this.schedulePersist();
    return id;
  }

  async updateProject(id: string, u: any) {
    await this.init();
    const keys = Object.keys(u);
    const set = keys.map(k => `${k.replace(/[A-Z]/g, l => `_${l.toLowerCase()}`)} = ?`).join(', ');
    this.db.exec({ sql: `UPDATE projects SET ${set} WHERE id = ?`, bind: [...Object.values(u), id] });
    this.schedulePersist();
  }
  ...
  async getProjectMembers(projectId: string): Promise<ProjectMember[]> {
    ...
    this.db.exec({
      sql: 'SELECT id, project_id, auth_uid, display_name, photo_url, role, joined_at FROM project_members WHERE project_id = ? ORDER BY joined_at ASC',
      bind: [projectId],
      callback: (r: any[]) => res.push({ id: r[0], projectId: r[1], authUid: r[2], displayName: r[3], photoUrl: r[4], role: r[5], joinedAt: r[6] })
    });
    return res;
  }

  async addProjectMember(projectId: string, authUid: string, displayName: string, photoUrl?: string | null, role?: string): Promise<string> {
    await this.init();
    const id = generateId();
    this.db.exec({
      sql: 'INSERT OR IGNORE INTO project_members (id, project_id, auth_uid, display_name, photo_url, role) VALUES (?, ?, ?, ?, ?, ?)',
      bind: [id, projectId, authUid, displayName, photoUrl || null, role || 'member']
    });
    this.schedulePersist();
    return id;
  }

  async removeProjectMember(projectId: string, authUid: string): Promise<void> {
    await this.init();
    this.db.exec({ sql: 'DELETE FROM project_members WHERE project_id = ? AND auth_uid = ?', bind: [projectId, authUid] });
    this.schedulePersist();
  }

  async getColumns(pid: string): Promise<Column[]> {
    await this.init();
    const res: Column[] = [];
    this.db.exec({ sql: 'SELECT * FROM columns WHERE project_id = ? ORDER BY order_index ASC', bind: [pid], callback: (r: any[]) => res.push({ id: r[0], projectId: r[1], name: r[2], orderIndex: r[3], isDeletable: !!r[4] }) });
    return res;
  }
  ...
  async getTasks(projectId?: string): Promise<Task[]> {
    await this.init();
    const res: Task[] = [];
    let sql = 'SELECT id, column_id, project_id, sprint_id, assignee_ids, title, description, tag, tag_variant, priority, progress, due_date, completed, is_archived, order_index, time_spent, is_timer_running, timer_started_at, created_at FROM tasks';
    const binds: any[] = [];
    if (projectId) { sql += ' WHERE project_id = ?'; binds.push(projectId); }
    sql += ' ORDER BY order_index ASC';
    this.db.exec({ sql, bind: binds, callback: (r: any[]) => res.push({ id: r[0], columnId: r[1], projectId: r[2], sprintId: r[3], assigneeIds: r[4] ? r[4].split(',').filter(Boolean) : [], title: r[5], description: r[6], tag: r[7], tagVariant: r[8], priority: r[9], progress: r[10], dueDate: r[11], completed: !!r[12], isArchived: !!r[13], orderIndex: r[14], timeSpent: r[15], isTimerRunning: !!r[16], timerStartedAt: r[17], createdAt: r[18] }) });
    return res;
  }
  ...
  async updateTaskColumn(tid: string, cid: string) {
    await this.init();
    let name = '';
    this.db.exec({ sql: 'SELECT name FROM columns WHERE id = ?', bind: [cid], callback: (r: any[]) => name = r[0] });
    const done = name.toLowerCase() === 'done';
    this.db.exec({ sql: `UPDATE tasks SET column_id = ?, completed = ? WHERE id = ?`, bind: [cid, done ? 1 : 0, tid] });
    this.schedulePersist();
  }
  ...
  async getSubtasks(tid: string): Promise<any[]> {
    await this.init();
    const res: any[] = [];
    this.db.exec({ sql: 'SELECT * FROM subtasks WHERE task_id = ? ORDER BY order_index ASC', bind: [tid], callback: (r: any[]) => res.push({ id: r[0], taskId: r[1], title: r[2], completed: !!r[3], orderIndex: r[4] }) });
    return res;
  }
  ...
  async getComments(tid: string): Promise<any[]> {
    await this.init();
    const res: any[] = [];
    this.db.exec({ sql: 'SELECT * FROM comments WHERE task_id = ? ORDER BY created_at DESC', bind: [tid], callback: (r: any[]) => res.push({ id: r[0], taskId: r[1], userId: r[2], content: r[3], createdAt: r[4] }) });
    return res;
  }
  ...
  async addComment(taskId: string, userId: string, content: string) {
    await this.init();
    const id = generateId();
    this.db.exec({ sql: 'INSERT INTO comments (id, task_id, user_id, content) VALUES (?, ?, ?, ?)', bind: [id, taskId, userId, content] });
    this.schedulePersist();
    return id;
  }

  async getActivities(pid: string): Promise<Activity[]> {
    await this.init();
    const res: Activity[] = [];
    this.db.exec({ sql: 'SELECT * FROM activities WHERE project_id = ? ORDER BY created_at DESC', bind: [pid], callback: (r: any[]) => res.push({ id: r[0], projectId: r[1], taskId: r[2], userId: r[3], type: r[4], content: r[5], createdAt: r[6] }) });
    return res;
  }
  ...
  async addActivity(a: any) {
    await this.init();
    const id = generateId();
    this.db.exec({ sql: 'INSERT INTO activities (id, project_id, task_id, user_id, type, content) VALUES (?, ?, ?, ?, ?, ?)', bind: [id, a.projectId, a.taskId || null, a.userId || null, a.type, a.content] });
    this.schedulePersist();
    return id;
  }
  ...
  async getSprints(projectId: string): Promise<Sprint[]> {
    ...
    const res: Sprint[] = [];
    this.db.exec({
      sql: 'SELECT id, project_id, name, start_date, end_date, status FROM sprints WHERE project_id = ? ORDER BY start_date ASC',
      bind: [projectId],
      callback: (r: any[]) => res.push({ id: r[0], projectId: r[1], name: r[2], startDate: r[3], endDate: r[4], status: r[5] as Sprint['status'] })
    });
    return res;
  }

  async addSprint(sprint: Omit<Sprint, 'id'>): Promise<string> {
    await this.init();
    const id = generateId();
    this.db.exec({
      sql: 'INSERT INTO sprints (id, project_id, name, start_date, end_date, status) VALUES (?, ?, ?, ?, ?, ?)',
      bind: [id, sprint.projectId, sprint.name, sprint.startDate, sprint.endDate, sprint.status || 'planned']
    });
    this.schedulePersist();
    return id;
  }
