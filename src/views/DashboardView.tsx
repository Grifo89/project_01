...
import { CheckCircle, Bolt, Rocket, Clock, ChevronDown, MoreVertical, Layout, Loader2 } from 'lucide-preact';
import { db, Task, Project, Activity } from '../services/db';
    ...
    <div className="flex-1 px-6 lg:px-10 py-8 overflow-y-auto custom-scrollbar pb-24 lg:pb-10">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          <StatCard title="Avg. Progress" value={`${avgProgress}%`} trend={avgProgress > 50 ? "+5%" : "-2%"} trendDirection={avgProgress > 50 ? "up" : "down"} icon={Layout} iconVariant="primary" />
          <StatCard title="Active Tasks" value={activeTasksCount.toString()} trend={activeTasksCount > 5 ? "+2" : "-1"} trendDirection={activeTasksCount > 5 ? "up" : "down"} icon={CheckCircle} iconVariant="amber" />
          <StatCard title="Completion" value={`${completionRate}%`} trend={completionRate > 70 ? "+8%" : "+2%"} icon={Bolt} iconVariant="emerald" />
          <StatCard title="Total Tasks" value={totalTasks.toString()} trend={totalTasks > 10 ? "+5" : "+1"} icon={Clock} iconVariant="primary" />
        </section>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card className="p-8">
              ...
              <div className="h-64 relative mt-10">
                <svg className="w-full h-full" viewBox="0 0 800 200" preserveAspectRatio="none">
                  ...
                  <path d="M0,150 Q100,150 200,120 T400,80 T600,100 T800,50" fill="none" stroke="var(--color-primary)" strokeWidth="4" strokeLinecap="round" />
                  <path d="M0,150 Q100,150 200,120 T400,80 T600,100 T800,50 L800,200 L0,200 Z" fill="url(#chartGradient)" className="opacity-20" />
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  onClick={() => route(`/project/${slugify(currentProject.name)}/board`)}
                ...
                {tasks.filter(t => !t.completed).slice(0, 3).map((task, i) => (
                  <Card key={i} className="flex items-center gap-4 p-5 rounded-2xl hover:bg-primary/10 hover:border-primary/30 transition-all duration-300 group cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-app-text-secondary text-xs mt-0.5">{task.tag} · Updated recently</p>
                    </div>
          ...
          <div className="space-y-8">
              ...
              <div className="space-y-8">
                {activities.length > 0 ? activities.slice(0, 5).map((activity, i) => (
                  <div key={activity.id} className="flex gap-4 relative">
                    {i !== Math.min(activities.length, 5) - 1 && <div className="absolute left-5 top-10 bottom-[-32px] w-px bg-app-border"></div>}
                    {/* Phase 0 placeholder avatar — replaced with real member avatar in Phase 1 */}
                    <div className="size-8 rounded-full bg-primary/20 flex-shrink-0" />
                    <div className="flex-1 space-y-1">
                      <p className="text-sm leading-snug">
                        <span className="font-bold text-app-text-primary">System</span>
            ...
            <Card className="p-6 bg-primary text-white border-none shadow-xl shadow-primary/20">
              ...
              <Button onClick={() => alert('Upgrade to Pro to unlock advanced features!')} className="w-full bg-white text-primary hover:bg-white/90 border-none">
                Upgrade Now
              </Button>
            </Card>
          </div>
