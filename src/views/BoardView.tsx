...
import { db, Task, Column, Project, Priority, User } from '../services/db';
...
const BoardView = ({ currentProject, searchQuery }: { currentProject: Project | null, searchQuery: string }) => {
  ...
  // Use a smaller distance threshold to allow clicking without accidentally dragging
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    if (activeTab === 'My Tasks') {
      matchesTab = task.assigneeIds?.includes(myUserId || '') || false;
    } else if (activeTab === 'Team Tasks') {
      matchesTab = (task.assigneeIds?.length || 0) > 0 && !task.assigneeIds?.every(id => id === myUserId);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Check if dropping over a column or a task in a different column
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-app-background">
      <div className="px-6 lg:px-10 py-6 space-y-6">
        ...
        <div className="flex items-center gap-6 border-b border-app-border">
          {['All Tasks', 'My Tasks', 'Team Tasks', 'Archived'].map((tab) => (
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragEnd={handleDragEnd} onDragStart={(e) => setActiveTask(tasks.find(t => t.id === e.active.id) || null)}>
              ...
              <KanbanColumn key={column.id} id={column.id} title={column.name} count={getTasksByColumn(column.id).length} color="bg-primary">
                <SortableContext items={getTasksByColumn(column.id).map(t => t.id)} strategy={verticalListSortingStrategy}>
                  {getTasksByColumn(column.id).map(task => (
                    <SortableTaskCard key={task.id} task={task} users={users} onClick={() => setSelectedTask(task)} />
                  ))}
                </SortableContext>
          ...
          {activeTask ? (
            <div className="opacity-80 rotate-3 scale-105 pointer-events-none">
              <TaskCard 
                ...
                assignees={activeTask.assigneeIds?.map(id => {
                  const u = users.find(user => user.id === id);
                  return u ? { initials: u.initials, src: u.avatarUrl } : null;
      ...
      <Modal isOpen={isTaskModalOpen} onClose={() => setIsTaskModalOpen(false)} title="Create New Task">
        <TaskForm users={users} onSubmit={handleAddTask} onCancel={() => setIsTaskModalOpen(false)} />
      </Modal>
      {selectedTask && (
        <TaskDetailModal 
...
const SortableTaskCard = ({ task, users, onClick }: { task: Task, users: User[], onClick: () => void }) => {
